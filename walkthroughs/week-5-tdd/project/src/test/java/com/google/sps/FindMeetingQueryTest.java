...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void optionalAttendeeWithOneEvent() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
  ...
  @Test
  public void optionalAttendeeWithSimilarDurationToFreeTime() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  : |-----A-----|     |-----B-----|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TimeRange.END_OF_DAY, false),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartDuration(TIME_0800AM, DURATION_1_HOUR),
            Arrays.asList(PERSON_C)));
  ...
  public void justEnoughRoomForOptionalAttendee() {
    ...
    public void optionalAttendeesWithGaps() {
        ...
        MeetingRequest request =
            new MeetingRequest(Collections.emptyList(), DURATION_30_MINUTES);
